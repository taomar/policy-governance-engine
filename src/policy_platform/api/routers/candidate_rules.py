...
async def list_policies(
    ...
    # One query for the whole page. Read, never generated: drawing a queue must
    # not cost a model call, and a policy with no stored label carries none
    # rather than acquiring one on the way to the screen.
    ...
    return [
        {
            ...
            # A short name for the subject, generated by this system, kept in
            # its own field under its own key so nothing merges it into the
            # document's words above. Absent when nobody has asked for one —
            # which the client shows as absent, not as blank.
            "topic_label": (
