...
**Budget generously.** Across the evaluation matrices run against this release, end-to-end decision times sat around **p50 19–26 s and p95 33–47 s**, with individual calls above that. The variation between matrices reflects the scenarios in them and the deep deployment they ran on, not the operation called: `/case/light` runs and stores the *same* decision as `/case` and is not a faster adjudicator — only its response is smaller. The configured deployment sits at the fast end: `gpt-5.6-terra` measured p50 20.8 s and p95 33.5 s, slowest call 38.8 s. These are observations over a fixed scenario set, not a service level objective; no latency guarantee is offered. The measurements behind them — including the fact that they were taken from a developer machine against Azure OpenAI in Sweden Central, so they carry public-internet round trips a co-located deployment would not — are in [Measured performance](measured-performance.md).
