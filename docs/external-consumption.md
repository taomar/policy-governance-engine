...
**Budget generously.** Across the evaluation matrices run against the current release, end-to-end decision times sat around **p50 22–32 s and p95 39–45 s**, with individual calls above that. The variation between matrices reflects the scenarios in them, not the operation called: `/case/light` runs and stores the *same* decision as `/case` and is not a faster adjudicator — only its response is smaller. These are observations over a fixed scenario set, not a service level objective; no latency guarantee is offered.
