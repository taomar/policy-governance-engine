...
| `embedding` | Embedding the query for semantic ranking. Runs **concurrently with the two probes above** — it needs only the scenario. Not run on `/policies`. Because it starts before the probes have answered, a request that is then refused for a missing or unprojected index has still paid for one embedding call; that is the cost of not waiting, and it is reported here rather than hidden. |
