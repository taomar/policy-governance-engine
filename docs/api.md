...
The current surface is **83 paths / 95 operations** across 12 tags.
...
| `ai` | `/api/ai` | 26 | Everything AI-assisted: status, ask, extract, extraction progress and runs, rewrite (+apply), rewrite preview, draft-from-text, scenario evaluation, compare, quality (published + candidates, each split into a POST that evaluates and a GET that reads the last result, plus history), policy-set summary, correlation runs/findings/dispositions, change explanation, generated subject names for the policies of a set, and a plain-words reading of one policy's extracted record. |
