...
The whole suite (`tests/`) **runs under `.[dev]` alone** — nothing fails at collection without the optional extra. Tests that need Docling carry a `pytest.mark.skipif` guard and skip themselves, naming the missing extra as the reason. The `[dev,graph]` extra is what makes those tests actually *execute* rather than skip; it is not required to collect or run the suite. See the [Docling integration](docling.md) guide for why it is kept separate.
...
- **`CORS_ALLOWED_ORIGINS=*`** — every browser origin is accepted. This is useful for a local integration playground and unsafe as a production default when callers send a shared subscription key. The current local worktree uses it; committed deployment examples do not.
...
All four policy-decision API operations — full Decision JSON, Decision Light, Policy JSON, and receipt replay — require a proved identity and refuse an unauthenticated caller with `401`, **independently of `RBAC_ENABLED`**. That is the only place the global flag is bypassed, and it only ever narrows access: decisions and receipts must name the principal that asked for them, while Policy JSON exposes approved policy records. Capability remains the global guard's decision.
...
| Authorization | A capability layer covering **all 108 API operations**. Each is classified into a band — read, use, author, administer — and one dependency enforces the registry, rather than checks scattered through routers. A guard test fails when any route is unclassified, and an unclassified route is denied at runtime too. **Disabled by default** (`RBAC_ENABLED=false`) so existing deployments are unchanged; see the note below before enabling it. The four policy-decision API operations (`/case`, `/case/light`, `/policies`, and receipt replay) are the exception: they additionally require a proved identity and answer `401` without one even when the flag is off. |
...
**Quality assurance is fail-closed.** Each rendering is checked structurally as it is produced — rejected if it is empty, implausibly larger or smaller than its source, or if a number or identifier failed to survive it, with the whole batch rejected on any failure. After a build, `POST /api/policy-sets/{key}/policy-index/validate` compares every projected document with its authoritative record under `policy-projection-quality-v1` and records `passed`, `failed`, or `unavailable`. Readiness requires `passed` under that exact profile; a successful rebuild without a recorded pass remains unmatchable. The live AIS and HW projections passed 138/138 and 115/115 documents with zero findings. The gate detects gross substitution but not every near-identical sibling-record swap; see [Known limitations](known-limitations.md).
...
| Docling tests skip with "optional 'graph' extra ... is not installed" | Expected: the `[dev]` extra does not include Docling, and those tests guard themselves rather than failing. | Not a bug. Install `.[dev,graph]` in a separate venv (`.venv-graph`) if you need them to execute — see the [Docling integration](docling.md) guide. |
