...
|---|---|---|
| Authorization is present but off by default | A capability layer classifies all 103 API operations and enforces them in one place, with bearer-token validation for both a local sign-in and an OIDC issuer. It ships behind `RBAC_ENABLED`, defaulting to false, so nothing is enforced until an operator turns it on and configures sign-in. | The build is not a security boundary as delivered. Configure sign-in and enable enforcement — see the checklist in [configuration](configuration.md) — before exposing it to untrusted users. |
