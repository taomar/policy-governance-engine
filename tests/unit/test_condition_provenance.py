...
class TestProvenanceClassification:
    def test_a_projected_condition_reports_derived(self) -> None:
        code, message = condition_provenance(_policy("when access is granted"), object())
        assert code == "derived"
    ...
    def test_stated_conditions_that_were_not_projected_are_distinguished(self) -> None:
        ...
        code, message = condition_provenance(_policy("for every emergency grant"), None)

        assert code == "conditions_not_projected"
        assert "for every emergency grant" in message
        assert "must not be treated as unconditional" in message

    def test_a_genuinely_unconditional_rule_is_distinguished(self) -> None:
        code, message = condition_provenance(_policy(None), None)

        assert code == "no_scope_derived"
        assert "may genuinely be" in message

    @pytest.mark.parametrize("blank", ["", "   ", "\n"])
    def test_whitespace_only_conditions_count_as_absent(self, blank: str) -> None:
        assert condition_provenance(_policy(blank), None)[0] == "no_scope_derived"

    def test_the_two_empty_tree_cases_never_share_a_code(self) -> None:
        dropped = condition_provenance(_policy("only during business hours"), None)[0]
        unscoped = condition_provenance(_policy(None), None)[0]
    ...
    def test_a_rule_without_a_canonical_rule_is_handled(self) -> None:
        ...
        assert condition_provenance(policy, None)[0] == "no_scope_derived"
...
class TestNoFabrication:
    def test_no_placeholder_condition_is_invented(self) -> None:
        ...
        _, message = condition_provenance(_policy("only for P1 incidents"), None)
