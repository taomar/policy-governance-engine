...
async def test_the_application_module_answers_the_reviewer_path_without_persisting(monkeypatch) -> None:
    """The delegating function is the decider's dict and nothing else.
    ...
    assert result == {"scope": "project", "evaluation": None}
...
def test_the_decision_statuses_named_by_the_contract_are_the_gathers_own() -> None:
    """The receipt's vocabulary is the decider's, plus one this layer adds.

    If `ai_case_intent` grew a status the envelope did not know, the projection
    would silently record it as `failed` — a real decision reported as a fault.
    This fails instead, at the moment the vocabularies diverge.
    """

    from policy_platform.application.policy_case_decision import _KNOWN_DECISION_STATUSES
    ...
    assert _KNOWN_DECISION_STATUSES == gather_statuses
